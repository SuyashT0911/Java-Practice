...
public class gameOnMultiDiementionalArray {
    public static void main(String[] args) {
        ...
        while (hits < 4) { // until the hits are less than four loop will continue executinng sice there is
                           // 4 targets
            ...
            System.out.println("Choose column number between 0 and 3: "); // it will select the columnwise position in
            ...
            if (ships[row][column]) { // testing condtion if selected target is true
                ...
                System.out.println("Hit!..." + (4 - hits) + "left.\n\n"); // show how many targets are left out of 4
                                                                          // after hit
            } else {
                System.out.println("Miss.\n\n"); // if selected co-ordinates are false then print target miss
            }
            numberOfTurns++; // increase number of turns by one after each try no matter if ship is hit or
                             // not
        }

        System.out.println("Victory"); // after successfully hitting all target ship diplay victory
        System.out.println("You won in " + numberOfTurns + " turns."); // after hitting all target show how much turns
                                                                       // user took to win
